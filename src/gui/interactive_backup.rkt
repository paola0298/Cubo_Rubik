...
(define (on-mouse state frames delta x_pos y_pos action)
    ;(printf "X: ~a Y: ~a Action: ~a\n" x_pos y_pos action)
    state)

(define (on-key state frames delta key_pressed)
...
(define (check-movement-input key_pressed)
    ...
    (cond 
        (movement-type-selected 
            (cond 
                (movement-direction
                    (printf "Mode: Columns  Direction: Left/Up     Index: ~a\n" movement-index))
                (else
                    (printf "Mode: Columns  Direction: Right/Down  Index: ~a\n" movement-index))
            ))
        (else 
            (cond 
                (movement-direction
                    (printf "Mode: Rows     Direction: Left/Up     Index: ~a\n" movement-index))
                (else
                    (printf "Mode: Rows     Direction: Right/Down  Index: ~a\n" movement-index))
            ))
    ))
